...
if exists("*searchpairpos")
	...
	if !exists('g:fennel_fuzzy_indent_patterns')
		let g:fennel_fuzzy_indent_patterns = ['^def', '^let', '^while', '^with', '^if', '^fn$', '^var$', '^case$', '^for$', '^each$', '^local$', '^global$', '^match', '^macro', '^lambda$', '^collect', '^icollect', '^accumulate', '^ns$']
	endif
    ...
    let str = split(a:string, '[.]')[-1]
		for pat in list
			if str =~# pat | return 1 | endif
		endfor
	endfunction
